$(function(){
    //把三个类型的input分别获取
    var $theadInput = $('thead input[type = checkbox]');//表头中全选框
    var $tbodyInputs = $('tbody input[type = checkbox]');//表格中的每一行选项框
    var $totalPriceInput = $('.totalPrice input[type = checkbox]');//计算总价中的全选框
    /*全选*/
    /*
        1.给表头中全选按钮 绑定点击事件 点击的时候 获取到它的选中状态(true/false)
        2.给表格中每一行的数据的input选择框，赋值为表头选中状态(true/false)
        3.给计算总价中全选框，赋值为表头的选中状态(true/false)
    */
   //表头的全选
   $theadInput.change(function(){
       var checkState = $(this).prop('checked');//获取全选框状态
       $tbodyInputs.prop('checked',checkState);//把状态表格中的选择框
       $totalPriceInput.prop('checked',checkState);//把状态计算总价全选框
       allTotal();//总计
   });
   /*
   计算总价的全选
       1.计算总价的全选按钮 绑定点击事件，获取选择状态(true/false)
       2.把状态给表头的全选
       3.把状态给表格中的选择框
   */
  ...
  /*
    ...
    5.把flag的值赋值给两个全选框(因为flag就对应选中状态)
  */
  $tbodyInputs.change(function(){//给表格单选框绑定事件
     ...
     $totalPriceInput.prop('checked',flag);//把状态赋值计算价格全选框
     allTotal();//总计
 })
 /*
       加法功能
 ...
 */
 ...
 function subTotal(dom,count){
     ...
     dom.closest('tr').find('.subprice').text(subTotalPrice.toFixed(2))//把小计结果渲染页面对应位置 保留2位小数

  
 }
 /*
     总计功能实现:(头部全选 计算总价全选 表格中选择框 + - 删除 六个地方调用总计)
        1.定义一个变量 用于保存总价 定义一个变量 用于保存已选商品 数量
        2.所有表格选择框 循环，获取选中状态 判断
        3.如果选中 那么就要累加一行的小计
  */
 function allTotal(){
     ...
     $('tbody input[type = checkbox]').each(function(){//获取表格中选择框 循环
         var checkState = $(this).prop('checked');//获取选中状态
